...
AUTH0_DOMAIN = 'capstonefsnd.uk.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'actor'
...
def get_token_auth_header():
    """
    Returns jwt token from request authorization header
    @return:
        str: jwt token
    """
    ...
    if not auth_header:
        raise AuthError({
            ...
            "description": "Missing authorisation header."
        }, 401)
    ...
    if len(header_parts) != 2 or header_parts[0].lower() != 'bearer':
        raise AuthError({
            ...
            "description": f"Malformed authorisation header: {auth_header}"
        }, 401)
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            ...
            "description": "Missing permissions in request header."
        }, 401)

    if permission not in payload['permissions']:
        raise AuthError({
            "code": "Forbidden",
            "description": f"Invalid permission. "
                           f"Permission: {permission} "
                           f"not in payload: {payload['permissions']}"
        }, 403)
